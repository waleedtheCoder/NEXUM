...
import { getAdminShopkeepers } from '../services/adminApi';
import { fonts, spacing, radii } from '../constants/theme';

export default function AdminShopkeepersScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const data = await getAdminShopkeepers();
        if (!cancelled) setShopkeepers(Array.isArray(data) ? data : []);
      } catch (err) {
        if (!cancelled) setError(err.message || 'Failed to load shopkeepers.');
      } finally {
  ...
  const filtered = useMemo(() => {
    if (!query.trim()) return shopkeepers;
  ...
  const renderItem = ({ item }) => {
    const initials = (item.name || '').split(' ').map((w) => w[0]).join('').slice(0, 2).toUpperCase() || 'K';
    return (
      <TouchableOpacity
        style={[styles.card, { backgroundColor: colors.surface }]}
        onPress={() => navigation.navigate('AdminUserDetail', { userId: item.id, userName: item.name, role: 'SHOPKEEPER', userData: item })}
        activeOpacity={0.82}
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.metaRow}>
            <View style={[styles.badge, { backgroundColor: `${colors.accent}15` }]}>
              ...
              <Text style={[styles.badgeText, { color: colors.accent }]}>{item.total_orders} orders</Text>
            </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScreenHeader title="Shopkeepers" subtitle={`${shopkeepers.length} registered`} showBack />
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              ...
              <Text style={[styles.emptyText, { color: colors.textSecondary }]}>No shopkeepers found.</Text>
            </View>
          }
        />
      )}
