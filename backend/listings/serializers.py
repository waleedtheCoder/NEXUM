...
class ListingDetailSerializer(serializers.ModelSerializer):
    ...
    def get_images(self, obj):
        # Return the single image_url as a one-element list so the image
        # carousel in ProductDetailScreen works without changes.
    ...
    def get_details(self, obj):
        return [
            ...
            {'label': 'Min. Order', 'value': f'25 {obj.unit}'},   # placeholder — add min_order field later
    ...
    def get_seller(self, obj):
        ...
        return {
            ...
            'rating': 4.8,   # placeholder until a ratings system exists
            'sales': 0,
...
class MyListingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Listing
        fields = [
            'id', 'productName', 'category', 'quantity', 'unit',
            'pricePerUnit', 'totalValue', 'status', 'imageUrl',
            'postedDate', 'location', 'views', 'inquiries',
        ]

    def get_quantity(self, obj):
        # Frontend stores this as a string (from the TextInput)
